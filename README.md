...
| Active and upcoming events | [Official Tibia.com Event Schedule](https://www.tibia.com/news/?subtopic=eventcalendar), always primary, with the Tibia client's own `eventschedule.json` as the single fallback | Fetched **at build time** by `lib/data/eventContentClient.ts`. The official client fetches five months around the reference month (two back, two forward, so periods reconstruct across month *and* year boundaries), joins overlapping calendar cells, and extracts the original title, plain-text tooltip description, source URL and complete period. A period runs from the server save of its first asterisked day to the server save of its last, so timestamps are 10:00 Europe/Berlin including DST, and active/upcoming transitions happen at the save rather than at midnight; overlapping events are kept independent and repeated occurrences of one title stay distinct. Each build fetches fresh pages once per month, with a 15-second timeout. The scheduled GitHub Actions rebuild runs weekly on Tuesdays at 12:00 Europe/Berlin (CET/CEST automatically). Pushes to main and manual workflow runs also rebuild and refresh the data. HTTP errors, bot challenges or relevant periods that cannot be closed produce a build warning and use `lib/data/eventschedule.json`, taken verbatim from the Tibia client and mapped into the same canonical shape by `lib/data/eventScheduleMapping.ts`. A calendar that parsed cleanly and simply has no events in range is a valid answer and never triggers the fallback. TibiaWiki and other third-party sources are **not** consulted for events at all — they never establish an event's existence, dates, times or active/upcoming status. `source` identifies the provider on every returned event. No proxy or browser scraping service is used. |
...
lib/
  ...
  data/          — worldProvider.ts (client hooks fetching TibiaData,
                   ...
                   HTML parsing, unit tested), eventschedule.json + eventScheduleMapping.ts /
                   eventScheduleFallback.ts (the Tibia client's own schedule, the single
                   fallback, unit tested), eventContentClient.ts (official-only event source)
