...
| Active and upcoming events | [Official Tibia.com Event Schedule](https://www.tibia.com/news/?subtopic=eventcalendar), always primary, read live when possible and otherwise from the capture of it committed at `lib/data/officialCalendar.json`; the Tibia client's own `eventschedule.json` is the last resort | Fetched **at build time** by `lib/data/eventContentClient.ts`, which reads three tiers of CipSoft data in order and never consults a third party. **1. The live page:** five months around the reference month (two back, two forward, so periods reconstruct across month *and* year boundaries), joined across overlapping calendar cells, with the original title, plain-text tooltip description, source URL and complete period. A period runs from the server save of its first asterisked day to the server save of its last, so timestamps are 10:00 Europe/Berlin including DST, active/upcoming transitions happen at the save rather than at midnight, overlapping events stay independent, and repeated occurrences of one title stay distinct. 15-second timeout per month. **2. The committed capture:** CipSoft serves the calendar behind a Cloudflare challenge that answers *every* unattended request, in GitHub Actions and locally alike, with HTTP 403 — so without a persisted copy the primary source contributes nothing to a real build and the app shows no events. `lib/data/officialCalendar.json` is the same page, read earlier through the same parser and written down. Refresh it with `npm run calendar:refresh` from a machine that can load the page, then commit the file; the scheduled build attempts the same refresh weekly and commits it when it succeeds, and leaves the last good capture untouched when it does not. **3. `lib/data/eventschedule.json`**, taken verbatim from the Tibia client and mapped by `lib/data/eventScheduleMapping.ts`, used only once the capture has run out of coverage; it lists the major seasonal events only. A calendar that parsed cleanly and simply has no events in range is a valid answer and never falls through. TibiaWiki and other third-party sources are **not** consulted for events at all. The scheduled GitHub Actions rebuild runs weekly on Tuesdays at 12:00 Europe/Berlin; pushes to main and manual runs also rebuild. No proxy or browser scraping service is used. |
...
lib/
  ...
  data/          — worldProvider.ts (client hooks fetching TibiaData,
                   ...
                   tibiaCalendarClient.ts / tibiaCalendarMapping.ts (official calendar HTTP +
                   HTML parsing, unit tested), officialCalendar.json +
                   officialCalendarStore.ts (the committed capture of that page, refreshed
                   by `npm run calendar:refresh`), eventschedule.json +
                   eventScheduleMapping.ts / eventScheduleFallback.ts (the Tibia client's
                   own schedule, last resort), eventContentClient.ts (the three CipSoft
                   tiers in order; no third party is ever consulted for events)
